...
public class FileParser {
    ...
    public FileParser(String processName) throws IOException {
        ...
        this.processedProjects = new HashSet<>();
                            ...
                            try {
                                ...
                                if (tokens.size() > MIN_TOKEN_IN_FILE) {
                                    ...
                                    for (String token : tokenToFrequencyMap
                                            .keySet()) {
                                        sb.append(token).append("@@::@@").append(tokenToFrequencyMap.get(token)).append(",");
                                    }
                                    if (sb.length() > 0) {
                                    ...
                                    System.out.println("ignoring file, num tokens can not match min token threshold: "
                                            + file.getAbsolutePath());
                                }
                            } catch (IOException e) {
    ...
    private Map<String, Integer> makeTokenToFrequencyMap(List<String> tokens) {
        Map<String, Integer> tokenToFrequencyMap = new HashMap<>();
