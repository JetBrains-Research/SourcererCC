                    ...
                    for entry in csvfile:
                        ...
                        if len(entry_split) == 9:
                            ...
                            if len(entry_split) % 2 != 0:
                                ...
                                logging.warning(
                                    'String partitioned into:' + file_id + '|' + proj_id + path + '|' + url + '|' + file_hash + '|' + bytess + '|' + lines + '|' + loc + '|' + sloc)
                        ...
                        if flag == 'files-autoID':
                            ...
                            db.insert_files_stats_ignore_repetition(file_hash, bytess, lines, loc, sloc,
                                                                    token_info[file_id][0], token_info[file_id][1],
                                                                    token_info[file_id][2])
                        else:
                ...
                with open(filename, 'r') as csvfile:
                    for entry in csvfile:
                        ...
                        if len(entry_split) == 8:
                            ...
                            db.insert_blocks_stats_ignore_repetition(block_hash, lines, loc, sloc, info[0], info[1],
                                                                     info[2])
            ...
            if filename.endswith('.stats'):
                ...
                with open(filename, 'r') as csvfile:
                    for entry in csvfile:
                        ...
                        db.insert_file(file_id, proj_id, path, url, file_hash)
                        db.insert_files_stats_ignore_repetition(file_hash, bytess, lines, loc, sloc, 'NULL', 'NULL',
                                                                'NULL')
...
if __name__ == "__main__":
    ...
    if flag not in ['blocks', 'files', 'files-autoID']:
        logging.error(
            'Usage: python mysql-import.py blocks|files|files-autoID user passwd database [output_path] [pairs_path]')
        sys.exit(1)
